...
__device__ inline void type1a_fb(curandState *rng, int *delta, const unsigned int *patch) {
    /*
...
#pragma unroll 4
    for (int li = 0; li < VECTORIZED_LIMIT; li += 4) {
        ...
        delta_vec.x += (patch_vec.x == 1);
        delta_vec.y += (patch_vec.y == 1);
        delta_vec.z += (patch_vec.z == 1);
        delta_vec.w += (patch_vec.w == 1);
        delta_vec.x -= (patch_vec.x == 0 && curand_uniform(rng) <= S_INV);
        delta_vec.y -= (patch_vec.y == 0 && curand_uniform(rng) <= S_INV);
        delta_vec.z -= (patch_vec.z == 0 && curand_uniform(rng) <= S_INV);
        delta_vec.w -= (patch_vec.w == 0 && curand_uniform(rng) <= S_INV);

        *((int4 *)&delta[li]) = delta_vec;
    }
...
#pragma unroll 4
    for (int li = VECTORIZED_LIMIT; li < LITERALS; ++li) {
        ...
        if (patch_bit == 1) {
            delta[li] += 1.0;
        } else if (patch_bit == 0 && curand_uniform(rng) < S_INV) {
            delta[li] -= 1.0;
        }
    }
}

__device__ inline void type1b_fb(curandState *rng, int *delta) {
    /*
...
#pragma unroll 4
    for (int li = 0; li < VECTORIZED_LIMIT; li += 4) {
        ...
        delta_vec.x -= (curand_uniform(rng) <= S_INV);
        delta_vec.y -= (curand_uniform(rng) <= S_INV);
        delta_vec.z -= (curand_uniform(rng) <= S_INV);
        delta_vec.w -= (curand_uniform(rng) <= S_INV);
...
#pragma unroll 4
    for (int li = VECTORIZED_LIMIT; li < LITERALS; ++li) {
        if (curand_uniform(rng) < S_INV) {
            delta[li] -= 1.0;
        }
    }
}

__device__ inline void type2_fb(int *delta, const unsigned int *patch) {
    /*
...
#pragma unroll 4
    for (int li = 0; li < VECTORIZED_LIMIT; li += 4) {
        ...
        delta_vec.x += (patch_vec.x == 0);
        delta_vec.y += (patch_vec.y == 0);
        delta_vec.z += (patch_vec.z == 0);
        delta_vec.w += (patch_vec.w == 0);
...
__global__ void calc_delta(curandState *rng, const unsigned int *global_ta_states, const float *clause_weights,
                           const float *class_sums, const int *selected_patch_ids, const int *num_includes,
                           const unsigned int *X_batch, const int *Y_batch, const int e, int *dstate1a, int *dstate1b,
                           int *dstate2, float *dw) {
    ...
    for (int clause = index; clause < CLAUSES; clause += stride) {
        ...
        // const unsigned int *ta_state = &global_ta_states[clause * LITERALS];
        int *local_dstate1a = &dstate1a[clause * LITERALS];
        int *local_dstate1b = &dstate1b[clause * LITERALS];
        int *local_dstate2 = &dstate2[clause * LITERALS];

        for (unsigned int class_id = 0; class_id < CLASSES; ++class_id) {
            if (tar[class_id] == -1 && curand_uniform(&localRNG) > Q_PROB) continue;  // Skip the class.

            const float *local_weight = &clause_weights[clause * CLASSES + class_id];
            int sign = (*local_weight >= 0) - (*local_weight < 0);
            bool should_update = (curand_uniform(&localRNG) <= prob[class_id]);
            ...
            if (should_update) {  // CLause update with prob update_p else skip
                if (type1 && local_clause_output) {
                    dw[clause * CLASSES + class_id] += sign * 1.0f;
#if (MAX_INCLUDED_LITERALS == LITERALS)
                    type1a_fb(&localRNG, local_dstate1a, patch);
#else
                    if (num_includes[clause] <= MAX_INCLUDED_LITERALS) type1a_fb(&localRNG, local_dstate1a, patch);
#endif

                } else if (type1 && !local_clause_output) {
                    type1b_fb(&localRNG, local_dstate1b);
                } else if (type2) {
                    dw[clause * CLASSES + class_id] -= sign * 1.0f;
                    type2_fb(local_dstate2, patch);
                }
            }
        }
    }
...
__global__ void update_weights(float *clause_weights, const float *dw) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;
    for (int clause = index; clause < CLAUSES; clause += stride) {
        for (int class_id = 0; class_id < CLASSES; ++class_id) {
            clause_weights[clause * CLASSES + class_id] += dw[clause * CLASSES + class_id];
        }
    }
}

__global__ void update_states(unsigned int *global_ta_states, const int *dstate1a, const int *dstate1b,
                              const int *dstate2) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;
    for (int clause_literal = index; clause_literal < CLAUSES * LITERALS; clause_literal += stride) {
        int clause = clause_literal / LITERALS;
        int literal = clause_literal % LITERALS;

        int total_change = dstate1a[clause * LITERALS + literal] + dstate1b[clause * LITERALS + literal] +
                           dstate2[clause * LITERALS + literal];

        int new_state = global_ta_states[clause * LITERALS + literal] + total_change;

        // if (new_state <= MAX_TA_STATE / 2)
        //     new_state += dstate2[clause * LITERALS + literal];

        // Clip the state to the range [0, MAX_TA_STATE]
        if (new_state > MAX_TA_STATE) {
            new_state = MAX_TA_STATE;
        } else if (new_state < 0) {
            new_state = 0;
        }
        global_ta_states[clause * LITERALS + literal] = (unsigned int)new_state;
    }
}
}
