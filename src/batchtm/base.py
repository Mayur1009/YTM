...
from pycuda.driver import (  # pyright: ignore[reportAttributeAccessIssue]
    device_attribute,  # pyright: ignore[reportAttributeAccessIssue]
    mem_alloc,  # pyright: ignore[reportAttributeAccessIssue]
    memcpy_dtoh,  # pyright: ignore[reportAttributeAccessIssue]
    memcpy_htod,  # pyright: ignore[reportAttributeAccessIssue]
    memset_d32,  # pyright: ignore[reportAttributeAccessIssue]
)
...
class BaseTM:
    ...
    def _fit_batch(self, X: csr_array, encoded_Y):
        ...
        dstate1a = mem_alloc(self.number_of_clauses * self.number_of_literals * 4)
        dstate1b = mem_alloc(self.number_of_clauses * self.number_of_literals * 4)
        dstate2 = mem_alloc(self.number_of_clauses * self.number_of_literals * 4)
        dw = mem_alloc(self.number_of_clauses * self.number_of_outputs * 4)
        ...
        memset_d32(dstate1a, 0, self.number_of_clauses * self.number_of_literals)
        memset_d32(dstate1b, 0, self.number_of_clauses * self.number_of_literals)
        memset_d32(dstate2, 0, self.number_of_clauses * self.number_of_literals)
        memset_d32(dw, 0, self.number_of_clauses * self.number_of_outputs)
        ...
        for e in range(N):
            ...
            self.kernel_calc_delta.prepared_call(
                *self.calc_delta_config,
                self.rng_gpu.state,
                self.ta_state_gpu,
                self.clause_weights_gpu,
                class_sum_gpu,
        ...
        # temp_dstate1a = np.empty((self.number_of_clauses, self.number_of_literals), dtype=np.int32)
        # temp_dstate1b = np.empty((self.number_of_clauses, self.number_of_literals), dtype=np.int32)
        # temp_dstate2 = np.empty((self.number_of_clauses, self.number_of_literals), dtype=np.int32)
        # temp_dw = np.empty((self.number_of_clauses, self.number_of_outputs), dtype=np.float32)
        # memcpy_dtoh(temp_dstate1a, dstate1a)
        # memcpy_dtoh(temp_dstate1b, dstate1b)
        # memcpy_dtoh(temp_dstate2, dstate2)
        # memcpy_dtoh(temp_dw, dw)
        # temp_dstate1a = temp_dstate1a.reshape((self.number_of_clauses, self.number_of_literals))
        # temp_dstate1b = temp_dstate1b.reshape((self.number_of_clauses, self.number_of_literals))
        # temp_dstate2 = temp_dstate2.reshape((self.number_of_clauses, self.number_of_literals))
        # temp_dw = temp_dw.reshape((self.number_of_clauses, self.number_of_outputs))
        # print(f'{temp_dstate1a=}')
        # print(f'{temp_dstate1b=}')
        # print(f'{temp_dstate2=}')
        # print(f'{temp_dw=}')
        #
        # breakpoint()

        self.kernel_update_weights.prepared_call(
            *self.update_weights_config,
            self.clause_weights_gpu,
            dw,
        )

        self.kernel_update_states.prepared_call(
            *self.update_states_config,
            self.ta_state_gpu,
            dstate1a,
            dstate1b,
            dstate2,
    ...
    def _gpu_init(self):
        ...
        self.kernel_calc_delta = mod_new_kernel.get_function("calc_delta")
        self.kernel_calc_delta.prepare("PPPPPPPPiPPPP")

        self.kernel_update_weights = mod_new_kernel.get_function("update_weights")
        self.kernel_update_weights.prepare("PP")

        self.kernel_update_states = mod_new_kernel.get_function("update_states")
        self.kernel_update_states.prepare("PPPP")
        ...
        self.calc_delta_config = kernel_config(
            self.number_of_clauses,
            self.device_props,
            self.block_size,
        )
        self.update_weights_config = kernel_config(
            self.number_of_clauses,
            self.device_props,
            self.block_size,
        )
        self.update_states_config = kernel_config(
